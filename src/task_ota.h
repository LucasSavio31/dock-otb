...
// =============================================================
//  taskOtaAutoCheck — verificação periódica e autônoma de releases
//  Core 1 | Prio 1 | Stack 10240 (WiFiClientSecure + HTTPClient + JSON)
//
//  Fluxo:
//    Boot +30s → verifica GitHub
//    Sucesso   → repete a cada 1h
//    Falha     → retry em 60s (max 5 tentativas, depois 1h)
//    Skip      → se taskOTA estiver ocupado (update/download em andamento)
// =============================================================
void taskOtaAutoCheck(void *param) {
  // Aguarda sistema e WiFi estabilizarem completamente
  vTaskDelay(pdMS_TO_TICKS(30000));

  uint8_t failCount = 0;

  for (;;) {
    // Não verifica enquanto taskOTA está executando um update ou download
    bool busy = false;
    if (xSemaphoreTake(mutexOta, pdMS_TO_TICKS(200)) == pdTRUE) {
      OtaStateEnum st = gOtaStatus.state;
      busy = (st == OTA_STATE_DOWNLOADING || st == OTA_STATE_FLASHING ||
              st == OTA_STATE_WIFI_CONNECTING || st == OTA_STATE_VALIDATING);
      xSemaphoreGive(mutexOta);
    }
    ...
    // Garante WiFi — reconecta se necessário
    if (!_otaEnsureWifi()) {
      vTaskDelay(pdMS_TO_TICKS(30000)); // retry em 30s sem WiFi
      continue;
    }
    ...
    if (found) {
      ...
      // Limpa estado done_err para não confundir o dashboard
      if (xSemaphoreTake(mutexOta, pdMS_TO_TICKS(200)) == pdTRUE) {
        if (gOtaStatus.state == OTA_STATE_DONE_ERR ||
            gOtaStatus.state == OTA_STATE_CHECKING) {
          gOtaStatus.state = OTA_STATE_IDLE;
      ...
      failCount++;
      uint32_t delay = (failCount >= 5) ? 3600000UL : 60000UL;
      vTaskDelay(pdMS_TO_TICKS(delay)); // 60s até 5 falhas, depois 1h
    }
  }
}
